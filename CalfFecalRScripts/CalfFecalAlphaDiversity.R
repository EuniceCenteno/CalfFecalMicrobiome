...
meta <- read.table("CalfFecal_metacomplete.txt", header=TRUE, row.names=2, sep="\t")
...
Ch <- ggplot(meta, aes(x=day, y=observed_otus, color=treatment)) + 
  geom_boxplot() + 
  labs(fill= "Treatment") +
  theme_bw()+
...
EV <- ggplot(meta, aes(x=day, y=pielou_e, color=treatment)) + 
  geom_boxplot() + 
  labs(fill= "Treatment") +
  theme_bw()+
