...
#Plot the graph 
ggplot(PhylumAB, aes(x = d, y = taxa.average, fill =Phylum)) + 
  ...
  ylab(paste0("Relative Abundance Phylum (Top 15)")) +  labs(x='Day')


### FAMILY LEVEL
num_genera <- 48 # we need 100 OTUs in order to get the 25 most abundant Genus
...
#Plot the graph 
ggplot(FamilyAB, aes(x = d, y = taxa.average, fill = Family)) + 
  geom_bar(stat = "identity") +
  theme_bw()+
  scale_fill_manual(values = my_colors) +
  facet_grid(treatment~.)+
  ...
  theme(axis.text.x = element_text(color = "black", size = 13), axis.text.y = element_text(color = "black", size = 13)) +
  #theme(text=element_text(family="Times New Roman")) +
  ylab(paste0("Relative Abundance Family (Top 15)")) +  labs(x='Day')


###----- TAXONOMY CALCULATION FOR TIME  
## The abundance at a family level grouped by time
num_genera <- 31 # we need 100 OTUs in order to get the 25 most abundant Genus
...
ggplot(GenusAB, aes(x = d, y = taxa.average, fill =Genus)) + 
  geom_bar(stat = "identity") +
  theme_bw()+
  scale_fill_manual(values = my_colors) +
  facet_grid(treatment~.)+
  ...
  theme(axis.text.x = element_text(color = "black", size = 13), axis.text.y = element_text(color = "black", size = 13)) +
  #theme(text=element_text(family="Times New Roman")) +
  ylab(paste0("Relative Abundance Genus (Top 15)")) +  labs(x='Day')
