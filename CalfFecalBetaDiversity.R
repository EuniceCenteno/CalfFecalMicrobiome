...
# Distance calculation using day 0 as the baseline 
distance <- read.table("calffecal_distances.txt", header=TRUE, row.names=1, sep="\t")
str(distance)
distance$treatment <- factor(distance$treatment)
distance$distance <- factor(distance$distance)
distance$d <- factor(distance$d)
levels(distance$distance)
levels(distance$d)

## Stats for treatment distances at each time point.
library(emmeans)

d0_28 <- subset(distance, subset=distance %in% c("0d-28d"))
#setting the glm model
anov_d0_28 <- lm(Wudist ~ treatment, data = d0_28)
#Running ANOVA
options(contrasts=c("contr.sum", "contr.poly"))

#Anova type III SS
ANOV_d0_28 <- Anova(anov_d0_28, type = 3)
ANOV_d0_28

mean_d0_28<-emmeans(anov_d0_28, ~ treatment)
mean_d0_28 <- data.frame(mean_d0_28)
mean_d0_28
comparison <- "0d-28d"
mean_d0_28$comparison <- comparison

d0_56 <- subset(distance, subset=distance %in% c("0d-56d"))
#setting the glm model
anov_d0_56 <- lm(Wudist ~ treatment, data = d0_56)
#Running ANOVA
options(contrasts=c("contr.sum", "contr.poly"))

mean_d0_56<-emmeans(anov_d0_56, ~ treatment)
mean_d0_56 <- data.frame(mean_d0_56)
mean_d0_56
comparison <- "0d-56d"
mean_d0_56$comparison <- comparison

#Anova type III SS
ANOV_d0_56 <- Anova(anov_d0_56, type = 3)
ANOV_d0_56

d0_84 <- subset(distance, subset=distance %in% c("0d-84d"))
#setting the glm model
anov_d0_84 <- lm(Wudist ~ treatment, data = d0_84)
#Running ANOVA
options(contrasts=c("contr.sum", "contr.poly"))

#Anova type III SS
ANOV_d0_84 <- Anova(anov_d0_84, type = 3)
ANOV_d0_84

mean_d0_84<-emmeans(anov_d0_84, ~ treatment)
mean_d0_84 <- data.frame(mean_d0_84)
mean_d0_84
comparison <- "0d-84d"
mean_d0_84$comparison <- comparison

d0_112 <- subset(distance, subset=distance %in% c("0d-112d"))
#setting the glm model
anov_d0_112 <- lm(Wudist ~ treatment, data = d0_112)
#Running ANOVA
options(contrasts=c("contr.sum", "contr.poly"))

#Anova type III SS
ANOV_d0_112 <- Anova(anov_d0_112, type = 3)
ANOV_d0_112

mean_d0_112<-emmeans(anov_d0_112, ~ treatment)
mean_d0_112 <- data.frame(mean_d0_112)
mean_d0_112
comparison <- "0d-112d"
mean_d0_112$comparison <- comparison

## Figures
d28 <- ggplot(mean_d0_28, aes(x=treatment, y=emmean, fill=treatment)) + 
...
d56 <- ggplot(mean_d0_56, aes(x=treatment, y=emmean, fill=treatment)) + 
  geom_bar(stat = "identity", alpha=0.8, colour="black") +
  geom_errorbar(aes(ymin = emmean - SE, ymax = emmean + SE), width = 0.2, position = position_dodge(width = 1)) + 
  scale_fill_manual(values = my_colors) +
  guides(fill=FALSE) +
...
d84 <- ggplot(mean_d0_84, aes(x=treatment, y=emmean, fill=treatment)) + 
  geom_bar(stat = "identity", alpha=0.8, colour="black") +
  geom_errorbar(aes(ymin = emmean - SE, ymax = emmean + SE), width = 0.2, position = position_dodge(width = 1)) + 
  scale_fill_manual(values = my_colors) +
  guides(fill=FALSE) +
...
d112 <- ggplot(mean_d0_112, aes(x=treatment, y=emmean, fill=treatment)) + 
  geom_bar(stat = "identity", alpha=0.8, colour="black") +
  geom_errorbar(aes(ymin = emmean - SE, ymax = emmean + SE), width = 0.2, position = position_dodge(width = 1)) + 
  scale_fill_manual(values = my_colors) +
  guides(fill=FALSE) +
...
ggarrange(d28, d56, d84, d112, labels = c("A", "B", "C", "D"),
